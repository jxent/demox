...
public interface Connection {
    /**
     * Returns the route used by this connection.返回这个connection的路由对象
     */
    Route route();
     ...
     *
     * 返回这个connection正在使用的socket对象。如果是https协议的话，返回的是SSLSocket对象。如果是
     * http/2协议的链接的话，这个socket可能被多个并发的call共享（多路复用）。
     */
    Socket socket();

    /**
     * Returns the TLS handshake used to establish this connection, or null if the connection is not
     * HTTPS.返回建立这个connection的TLS handshake对象，如果connection不是HTTPS的话，返回null。
     */
    Handshake handshake();
     ...
     * 返回这个connection协商的协议，如果没有协议被协商的话，返回HTTP_1_1，这个方法会返回HTTP_1_1，即便是
     * 远程另一端使用的是HTTP_1_0。
     */
    Protocol protocol();
}
